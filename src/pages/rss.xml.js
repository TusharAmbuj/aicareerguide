...
export async function GET(context) {
  ...
  const blog = await getCollection('blog', ({ data }) => !data.draft && data.status === 'published');
  const projects = await getCollection('projects', ({ data }) => !data.draft && data.status === 'published');
